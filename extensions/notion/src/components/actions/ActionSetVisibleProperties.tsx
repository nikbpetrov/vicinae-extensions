import { ActionPanel, Color, Action, Icon } from "@vicinae/api";
...
export function ActionSetVisibleProperties(props: {
	...
	const selectedProperties = selectedPropertiesIds.map((id) =>
		databaseProperties.find((dp) => dp.id === id),
	);
	const unselectedProperties = databaseProperties.filter(
		(dp) => !selectedPropertiesIds.includes(dp.id),
	);
