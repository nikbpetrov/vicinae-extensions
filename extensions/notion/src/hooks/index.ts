...
export function useVisibleDatabasePropIds(
	...
	if (quicklinkProps) {
		...
		const setVisiblePropIds = (props?: string[]) =>
			setDatabaseView({ ...data, create_properties: props });
		return {
			visiblePropIds: data.create_properties,
			isLoading,
			setVisiblePropIds,
		};
	}
...
export function useDatabasesView(databaseId: string) {
	const { data, isLoading, mutate } = useCachedPromise(async () => {
		...
		return JSON.parse(data) as {
			[databaseId: string]: DatabaseView | undefined;
		};
	});
	...
	async function setDatabaseView(view: DatabaseView) {
		...
		await LocalStorage.setItem(
			"DATABASES_VIEWS",
			JSON.stringify({ ...data, [databaseId]: view }),
		);
		mutate();
