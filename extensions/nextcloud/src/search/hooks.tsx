...
export function useSearch() {
	...
	const { data, isLoading } = useQuery(
		(signal) => performSearch(signal, query),
		[query],
	);
...
async function performSearch(
	...
	const availableItems = items.filter((item) => {
		const propStat =
			item["d:propstat"] instanceof Array
				? (item["d:propstat"] as propStat[])[0]
				: item["d:propstat"];
		return propStat["d:status"].includes(`200 OK`);
	});

	const res = availableItems.map(
		(item: { "d:href": string; "d:propstat": propStat | propStat[] }) => {
