...
export function PageListItem({
	...
	const visiblePropertiesIds: string[] =
		databaseProperties
			?.filter((dp: DatabaseProperty) => databaseView?.properties?.[dp.id])
			.map((dp) => dp.id) || [];
	...
	const { primaryAction, open_in } =
		getPreferenceValues<Preferences.SearchPage>();
						...
						<ActionPanel.Section title="View options">
							...
							<ActionSetVisibleProperties
								...
								onUnselect={(propertyId: string) => {
									...
									const { [propertyId]: _, ...remainingProperties } =
										databaseView?.properties ?? {};
		...
		case "multi_select":
			return property.value.map((option) => {
				return {
					tag: {
						value: option.name,
						color: notionColorToTintColor(option.color),
					},
