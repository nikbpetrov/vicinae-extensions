...
export function useRecentEntries() {
	...
	useEffect(() => {
		...
		async function loadEntries() {
			...
			try {
				...
				const result: any = await execFilePromise("sqlite3", [dbPath, query], {
					encoding: "utf8",
				});
				...
				if (result && typeof result === "object" && "stdout" in result) {
					...
					resultStr =
						typeof stdout === "string" ? stdout.trim() : String(stdout).trim();
	...
	async function removeEntry(entry: EntryLike) {
		...
		try {
			await saveEntries(
				data.filter(
					(currentEntry: EntryLike) => !isSameEntry(currentEntry, entry),
				),
			);
			await revalidate();
			showToast(
				Toast.Style.Success,
				"Entry removed",
				`Restart Cursor to sync the list in Cursor (optional)`,
			);
		} catch (error) {
