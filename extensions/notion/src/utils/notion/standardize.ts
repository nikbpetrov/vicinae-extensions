...
export type Standardized<
	UnionObject extends { type: string },
...
> = {
	...
	[Obj in UnionObject as Obj["type"]]: Prettify<
		Pick<Obj, keyof UnionObject> & {
			...
			[K in NewKey]: Omit<Obj, keyof UnionObject>[keyof Omit<
				Obj,
				keyof UnionObject
			>] extends { type: string }
				? Standardized<
...
export function standardize<
	UnionObject extends { type: string },
	NewKey extends string,
>(object: UnionObject, newKey: NewKey): Standardized<UnionObject, NewKey> {
	return recursiveStandardize(object, newKey) as Standardized<
		UnionObject,
		NewKey
	>;
}
function recursiveStandardize(object: unknown, newKey: string) {
	if (isTyped(object)) {
		...
		for (const key in object)
			if (key == object.type)
				newObject[newKey] = recursiveStandardize(object[key], newKey);
...
function isTyped<T extends string>(
	value: unknown,
): value is { type: T } & { [K in T]: unknown } {
	return (
