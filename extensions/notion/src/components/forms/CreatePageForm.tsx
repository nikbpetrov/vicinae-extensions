...
export function CreatePageForm({
	...
	const { itemProps, values, handleSubmit, reset, focus } =
			...
			async onSubmit(values) {
				...
				try {
					...
					if (mutate) {
						...
						reset(initialValues);
						const titleProperty = databaseProperties?.find(
							(dp) => dp.type == "title",
						);
	...
	const renderSubmitAction = (type: "main" | "second") => {
		...
		if (
			(!preferences.closeAfterCreate && type === "main") ||
			(preferences.closeAfterCreate && type === "second")
		) {
			return (
				<Action.SubmitForm
					title="Create Page"
	...
	return (
		<Form
			...
			navigationTitle={
				initialDatabaseId ? "Create New Page" : "Create Database Page"
			}
			actions={
				<ActionPanel>
					<ActionPanel.Section>
					...
					{databaseProperties ? (
						<ActionPanel.Section title="View options">
							<ActionSetVisibleProperties
								databaseProperties={databaseProperties.filter(
									(dp) => dp.id !== "title",
								)}
								...
								onSelect={(propertyId) =>
									setVisiblePropIds(
										visiblePropIds
											? [...visiblePropIds, propertyId]
											: [propertyId],
									)
								}
								onUnselect={(propertyId) =>
									setVisiblePropIds(
										(visiblePropIds || databasePropertyIds).filter(
											(pid) => pid !== propertyId,
										),
									)
								}
