...
export function DatabaseViewForm(props: {
	...
	async function handleSubmit(values: Form.Values) {
		...
		if (values.type === "kanban") {
			newDatabaseView.kanban = {
				...
				backlog_ids: values["kanban::backlog_ids"]
					? values["kanban::backlog_ids"]
					: [],
				not_started_ids: values["kanban::not_started_ids"]
					? values["kanban::not_started_ids"]
					: [],
				started_ids: values["kanban::started_ids"]
					? values["kanban::started_ids"]
					: [],
				completed_ids: values["kanban::completed_ids"]
					? values["kanban::completed_ids"]
					: [],
				canceled_ids: values["kanban::canceled_ids"]
					? values["kanban::canceled_ids"]
					: [],
			};
		}
	...
	const [viewType, setViewType] = useState<"kanban" | "list">(
		databaseView?.type ? databaseView.type : "list",
	);
	const { data: databaseProperties, isLoading: isLoadingDatabaseProperties } =
		useDatabaseProperties(databaseId);

	useEffect(() => {
		if (databaseProperties && viewType === "kanban") {
			const hasSelect = databaseProperties.some(
				(dp) => dp.type === "select" || dp.type === "status",
			);
			...
			{databaseProperties && viewType === "kanban" ? (
				<KanbanViewFormItem
					...
					properties={databaseProperties.filter(
						(dp): dp is StatusDatabaseProperty =>
							isType(dp, "select", "status"),
					)}
				/>
			) : null}
		</Form>
...
type StatusDatabaseProperty = Extract<
	DatabaseProperty,
	{ type: "select" | "status" }
>;
...
function KanbanViewFormItem({
	properties,
	databaseView,
}: KanbanViewFormItemParams) {
	...
	function getStatusState(property: DatabaseProperty | undefined) {
		...
		const statusOptions = property.config.options.filter(
			(o) => o.id !== "_select_null_",
		);
		const currentConfig = databaseView?.kanban;

		const defaultBacklogOpts = currentConfig
			? currentConfig.backlog_ids
			: ["_select_null_"];
		...
		const defaultStartedOpts = currentConfig
			...
			: statusOptions
					.filter(
						(o) =>
							!defaultNotStartedOpts.includes(o.id!) &&
							!defaultCompletedOpts.includes(o.id!),
					)
					.map((o) => o.id!);
			...
			? Object.keys(statusTypes).map((statusTypeId) => (
					<StatusTagPicker
						...
						onChange={(newValue: string[]) =>
							setStatusState((x) => ({ ...x, [statusTypeId]: newValue }))
						}
					/>
				))
			: [],
			...
			{statusProperty.config.options.map((o) => {
				...
				return (
					<Form.TagPicker.Item
						...
						title={o.name}
						icon={{
							source: `./icon/kanban_status_${id}.png`,
							tintColor: notionColorToTintColor(o.color),
						}}
					/>
				);
			})}
