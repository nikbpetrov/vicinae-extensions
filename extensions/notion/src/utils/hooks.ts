...
export function useCachedPromise<T, U extends any[] = []>(
  fn: ((...args: U) => Promise<T>) | ((...args: U) => (cursor?: any) => Promise<{ data: T; hasMore: boolean; cursor?: any }>) | (() => Promise<T>),
  args?: U | [],
  ...
  const [paginationState, setPaginationState] = useState<{ hasMore: boolean; cursor?: any }>({ hasMore: false });
  ...
  const fetchData = useCallback(async (cursor?: any) => {
    ...
    try {
      ...
      if (typeof fn === 'function' && fn.length > 0) {
        const paginationFn = args && args.length > 0 
          ? (fn as (...args: U) => (cursor?: any) => Promise<{ data: T; hasMore: boolean; cursor?: any }>)(...(args as U))
          : null;
        ...
        if (paginationFn && typeof paginationFn === 'function') {
          ...
          setPaginationState({ hasMore: result.hasMore, cursor: result.cursor });
        } else {
...
export function useForm<T extends Record<string, any>>(options: {
  ...
  const validate = useCallback((vals: Partial<T>): boolean => {
  ...
  const handleSubmit = useCallback(async (vals: T) => {
    if (!validate(vals)) {
      return;
    }
  ...
  const allKeys = new Set([...Object.keys(options.initialValues || {}), ...Object.keys(values)]);
