import { Form, Icon, Image } from "@vicinae/api";
...
export function PagePropertyField({
	...
	const sharedProps = {
		title: databaseProperty.name,
		placeholder: createPlaceholder(databaseProperty),
	};
	switch (type) {
		case "date":
			return (
				<Form.DatePicker {...itemProps} {...sharedProps} key={itemProps.id} />
			);
		case "checkbox":
			return (
				<Form.Checkbox
		...
		case "status":
			return (
				<Form.Dropdown {...itemProps} {...sharedProps} key={itemProps.id}>
					{databaseProperty.config.options.map(
		...
		case "people": {
			...
			if (databaseProperty.type == "multi_select")
				options = databaseProperty.config.options;
...
function createMapOptionsFunc(
	Tag: typeof Form.Dropdown.Item | typeof Form.TagPicker.Item,
) {
	return (option: ItemOption | Page | User) => {
		...
		if ("type" in option) {
			title = option.name;
			icon = option.avatar_url
				? { source: option.avatar_url, mask: Image.Mask.Circle }
				: undefined;
			...
			title = option.name;
			icon = option.color
				? { source: Icon.Dot, tintColor: notionColorToTintColor(option.color) }
				: undefined;
		}
		return (
			<Tag
