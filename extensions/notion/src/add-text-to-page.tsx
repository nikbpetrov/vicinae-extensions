...
namespace Arguments {
	export interface AddTextToPage {
		text?: string;
	}
}

function AddTextToPage(
	props: LaunchProps<{ arguments: Arguments.AddTextToPage }>,
) {
		...
		async onSubmit(values) {
			try {
				await showToast({
					...
					title: "Adding content to the page",
				});

				const selectedPage = searchPages?.find(
					(page) => page.id === values.page,
				);

				if (!selectedPage) {
					await showToast({
						style: Toast.Style.Failure,
						title: "Could not find the selected page",
					});
					return;
				}

				const content = markdownToBlocks(
					values.textToAppend,
				...
				await showToast({
					...
					title: "Added text to page",
				});
			} catch (error) {
				await showToast({
					style: Toast.Style.Failure,
					title: "Failed adding text to page",
				});
			}
		},
		initialValues: {
	...
	return (
		...
		>
			...
			<Form.TextArea
				enableMarkdown
				autoFocus={!props.arguments.text}
				title="Content"
				{...itemProps.textToAppend}
			/>
