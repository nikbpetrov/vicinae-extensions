...
function pagePropertyToText(
	property:
		| PageProperty
		| (Extract<PageProperty, { type: "formula" }>["value"] & { id: string }),
): string | undefined {
									...
									<Detail.Metadata.Label
										...
										icon={
											lastEditedUser?.avatar_url
												? {
														source: lastEditedUser.avatar_url,
									...
									<Detail.Metadata.Label
										...
										icon={
											createdByUser.avatar_url
												? {
														source: createdByUser.avatar_url,
														mask: Image.Mask.Circle,
													}
												: undefined
										}
					...
					<ActionPanel>
						...
						<ActionPanel.Section>
							<Action.Push
								...
								target={
									<AppendToPageForm page={page} onContentUpdate={mutate} />
								}
							/>
						</ActionPanel.Section>
						<ActionPanel.Section>
	...
	switch (property.type) {
		...
		case "date": {
			...
			if (property.value.end)
				displayedDate += ` → ${format(new Date(property.value.end), "MMMM d, yyyy")}`;
			return (
				<Detail.Metadata.Label
					key={property.id}
					title={title}
		...
		case "title":
			return property.value.length > 0 ? (
				<Detail.Metadata.Label
					key={property.id}
					title={title}
					text={property.value[0]?.plain_text}
				/>
			) : null;
		case "select":
		case "status":
