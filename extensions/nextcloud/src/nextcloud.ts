...
export function useQuery<R>(
	...
	const [state, setState] = useState<{ data: R | null; isLoading: boolean }>({
		data: null,
		isLoading: true,
	});
...
export async function webdavRequest({
	...
	if (response.status === 401) {
		throw new Error(
			"Authentication failed. Please check your username and app password in extension preferences.",
		);
	}

	if (!response.ok) {
		throw new Error(
			`HTTP ${response.status}: ${response.statusText}. Response: ${responseBody.substring(0, 200)}`,
		);
	}
