...
type EditPropertyOptions = PropertyConfig<
	"select" | "multi_select"
>["options"][number] & {
	icon?: string;
};
...
export function ActionEditPageProperty({
	...
	const { type, config, value } = propertyHelper(
		databaseProperty,
		pageProperty,
	);
				...
				<ActionPanel.Submenu title={title} icon={icon} shortcut={shortcut}>
					{options?.map((opt) => (
						<Action
							...
							title={
								(opt.name ? opt.name : "Untitled") +
								(opt.icon && value?.id === opt.id ? "  ✓" : "")
							}
							onAction={() => {
								if (opt.id && opt.id !== "_select_null_") {
									setPageProperty({
										[databaseProperty.id]: { select: { id: opt.id } },
									});
								} else {
					...
					{options?.map((opt) => (
						<Action
							...
							onAction={() => {
								if (opt.id && opt.id !== "_select_null_") {
									setPageProperty({
										[databaseProperty.id]: { status: { id: opt.id } },
									});
								} else {
			...
			return (
				<ActionPanel.Submenu title={title} icon={icon} shortcut={shortcut}>
					<ActionPanel.Submenu
						title={
							value?.start
								? formatDistanceToNow(new Date(value.start))
								: "No Date"
						}
					...
					<ActionPanel.Submenu
						title={
							value?.end ? formatDistanceToNow(new Date(value.end)) : "No Date"
						}
					...
					{options?.map((opt) => {
						...
						return (
							<Action
								...
								icon={{
									source:
										opt.id && multiSelectIds?.includes(opt.id)
											? Icon.Checkmark
											: Icon.Circle,
									tintColor: notionColorToTintColor(opt.color),
								}}
