import { List, Image } from "@vicinae/api";
...
export function DatabaseView(props: DatabaseViewProps) {
	...
	function statusSourceIcon(dspoId: string) {
		...
		if (startedIds.includes(dspoId)) {
			...
			const currentStatus = Number.parseFloat(
				(statusIndex / statusSize).toFixed(2),
			);
			let percent = "25";
		...
		if (completedIds.includes(dspoId))
			source_icon = "icon/kanban_status_completed.png";

		if (canceledIds.includes(dspoId))
			source_icon = "icon/kanban_status_canceled.png";
	...
	const optionsMap: Record<
		string,
		PropertyConfig<"status">["options"][number]
	> = {};
	...
	databaseSections?.map((ds) => {
		SectionElement.push(
			<List.Section
				...
				title={ds.name}
				subtitle={
					ds?.pages?.length + (ds?.pages?.length > 1 ? " Items" : " Item")
				}
			>
				{ds?.pages?.map((p) => (
					<PageListItem
